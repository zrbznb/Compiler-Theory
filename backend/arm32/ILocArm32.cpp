...
std::string ArmInst::outPut()
{
    ...
    // 第一元参数输出
    if (!arg1.empty()) {
        // 修改：第一个参数应该用空格分隔（如果结果为空）或逗号分隔（如果结果不为空）
        if (result.empty()) {
            ret += " " + arg1;
        } else {
            ret += "," + arg1;
...
{
    // 检查是否是需要两个操作数的指令
    if (op == "mov" || op == "add" || op == "sub" || op == "mul" || op == "ldr" || op == "str") {
        // 对于需要两个操作数的指令，如果只提供了一个，添加一个占位符或报错
        minic_log(LOG_ERROR, "指令 %s 需要至少两个操作数，但只提供了一个: %s", op.c_str(), rs.c_str());
    }
    emit(op, rs);
}
...
{
    // 确保指令格式正确，使用空格分隔操作码和操作数
    std::string instStr = op + " " + rs + "," + arg1;
    if (!arg2.empty()) {
        instStr += "," + arg2;
    }
