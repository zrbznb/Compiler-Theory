...
void Function::toString(std::string & str)
{
    ...
    for (auto & var: this->varsVector) {
        // 是否是数组变量
        bool isArrayVar = var->getType()->isArrayType();
...
ArrayVariable* Function::newArrayVariable(ArrayType* arrayType, const std::string& name, int32_t scopeLevel) {
    ArrayVariable* arrayVar = new ArrayVariable(arrayType, name, scopeLevel);
    
    // 将数组变量添加到变量列表中
    varsVector.push_back(arrayVar);
    
    return arrayVar;
}

ArrayParamVariable* Function::newArrayParamVariable(ArrayType* arrayType, const std::string& name) {
    ArrayParamVariable* arrayParam = new ArrayParamVariable(arrayType, name);
    
    // 将数组参数添加到变量列表中
    varsVector.push_back(arrayParam);
